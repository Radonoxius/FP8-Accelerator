...
pub fn divide(a: &Fp8, b: &Fp8) -> (Fp8, State) {
    ...
    let a_mant_full = a_mant as u16;
    let b_mant_full = b_mant as u16;

    // Shift dividend up by 10 to allow sufficient precision for division and rounding
    let dividend = a_mant_full << 10;
    let raw_quot = dividend / b_mant_full;
    let rem = dividend % b_mant_full;
    ...
    while result_exp < -6 && full_mant > 0 {
        lost_bit = lost_bit || ((full_mant & 1) != 0);
        full_mant >>= 1;
        result_exp += 1;
    }

    // Perform Round-to-Nearest, Ties to Even
    let guard = (full_mant >> 2) & 1;
    let sticky = (full_mant & 0b11) != 0 || lost_bit || rem != 0;
    let truncated = (full_mant >> 3) & 0b11111;

    let round_up = guard == 1 && (sticky || (truncated & 1) == 1);
    let mut abs_mant = truncated + if round_up { 1 } else { 0 };

    // Renormalize results in case rounding caused a carry out
    if abs_mant >= 16 {
        abs_mant >>= 1;
        result_exp += 1;
    }
    ...
    // Return signed 0 on underflow
    if result_exp < -6 {
        return (Fp8::new(result_sign, 0, 0), State::Zero);
    }
