module fp8_adder(
    ...
    // sign bits - positive if sign bit is 0
    wire a_pos = ~A[7];
    wire b_pos = ~B[7];
    ...
    // NaN is 0x7F in E4M3, zero is when lower 7 bits are all 0
    ...
    always @(*) begin
        ...
        if (unbiased_a > unbiased_b) begin
            ...
            res_exp = unbiased_a;
            // same exponent, check which mantissa is bigger
            if (av >= bv) begin
                if (a_pos && b_pos) begin
                ...
                // B mantissa is bigger so swap order
        ...
        // overflow into bit 4, shift right and bump exp
